...
const getPackageInfo = async () => {
  try {
    setLoading(true);
    const response = await axios.get('https://uat.zippyrideadminapi.projectpulse360.com/GetPackageInfo', {
      params: {
    ...
    // Success case - populate form with existing data
    ...
    if (response.data && response.data.length > 0) {
      ...
      // Set city values directly without triggering suggestions
      setFromCity(packageData.fromcity || "");
      setToCity(packageData.tocity || "");
      
      setOneWayDrop(packageData.isOneway === true || packageData.isOneway === "true");
      setTwoWayDrop(packageData.isTwoWay === true || packageData.isTwoWay === "true");
      setOneWayPrice(packageData.onewayprice ? packageData.onewayprice.toString() : "");
      setTwoWayPrice(packageData.twowayprice ? packageData.twowayprice.toString() : "");
    }
    ...
    Alert.alert("Error", "Failed to load package data. Please try again.");
  } finally {
    setLoading(false);
  }
};

// API call to update package using Axios with FormData
const updatePackage = async (formData) => {
    console.log(formData,'formData')
  setLoading(true);
  try {
    const response = await axios.post(
      `${config.BASE_URL}UpdateOutstationPackage`, 
      formData, 
      {
        headers: {
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      const formData = new FormData();
       formData.append('PackageID', route?.params?.packageId);
      ...
      // Add PackageID for update operation if we're editing
      if (route?.params?.packageId) {
        formData.append('PackageID', route.params.packageId);
      }
      ...
      if (route?.params?.packageId) {
        ...
        result = await updatePackage(formData);
        setSuccessMessage("Trip Package updated successfully!");
      } else {
        // Create new package
      ...
      Alert.alert(
        "Error", 
        error.message || `Failed to ${route?.params?.packageId ? 'update' : 'create'} package. Please try again.`
      );
    }
  };
  ...
  const handleSuccessModalClose = () => {
    ...
    if (route?.params?.packageId) {
      navigation.goBack(); // Go back to previous screen after successful edit
    } else {
      resetForm();
    }
  };
  ...
  // Function to manually update from city (for external changes)
  const updateFromCity = (city) => {
    setFromCity(city);
    // Clear suggestions when setting city programmatically
    setFromSuggestions([]);
  };

  // Function to manually update to city (for external changes)
  const updateToCity = (city) => {
    setToCity(city);
    // Clear suggestions when setting city programmatically
    setToSuggestions([]);
  };
  ...
  // Handle suggestion selection for From City
  ...
  // Handle suggestion selection for To City
       ...
       >
         ...
         >
           <Text style={styles.header}>
             {route?.params?.packageId ? "Edit Package" : "Update Package"}
           </Text>
           ...
           <View style={[styles.inputContainer, errors.fromCity && styles.inputError]}>
             ...
             />
             {fromCity.length > 0 && (
               <TouchableOpacity onPress={clearFromCity} style={styles.clearButton}>
                 <Svg width={scale(16)} height={scale(16)} viewBox="0 0 24 24" fill="none">
                   <Path
               ...
               <FlatList
                 ...
                 renderItem={({ item }) => (
                   <TouchableOpacity
                     ...
                     onPress={() => handleFromSuggestionSelect(item)}
                   >
           ...
           <View style={[styles.inputContainer, errors.toCity && styles.inputError]}>
             ...
             />
             {toCity.length > 0 && (
               <TouchableOpacity onPress={clearToCity} style={styles.clearButton}>
                 <Svg width={scale(16)} height={scale(16)} viewBox="0 0 24 24" fill="none">
                   <Path
                     d="M18 6L6 18M6 6l12 12"
                     stroke="#999"
             ...
             <View style={styles.suggestionsContainer}>
               <FlatList
                 ...
                 renderItem={({ item }) => (
                   <TouchableOpacity
                     style={styles.suggestionItem}
                     onPress={() => handleToSuggestionSelect(item)}
                   >
                     <Text style={styles.suggestionText}>{item.description}</Text>
                   </TouchableOpacity>
                 )}
           ...
           >
             {loading ? (
               ...
               <Text style={styles.submitText}>
                 {route?.params?.packageId ? "Update Package" : "Create Package"}
               </Text>
             )}
           </TouchableOpacity>
         </ScrollView>
         ...
         <View style={styles.loadingOverlay}>
           <View style={styles.loadingContainer}>
             ...
             <Text style={styles.loadingText}>
               {route?.params?.packageId ? "Updating Package..." : "Update Package..."}
             </Text>
           </View>
         </View>
       )}
