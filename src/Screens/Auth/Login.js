...
const SignInScreen = () => {
  ...
  const isLoginCount =   useSelector(state => state.auth.isLoginCount);
    ...
    const [firstlogin, setFirstLogin] = useState(getItem('firstimelogin') || 'true');
 useEffect(() => {
  ...
  useEffect(() => {
    ...
    const HIGH_ACCURACY = Platform.OS === 'android' ? {
    ...
    const handleLocationSuccess = (position) => {
      ...
      if (latitude !== locationRef.current.latitude || 
  ...
  const SignUpSchema = Yup.object().shape({
    mobileOrEmail: Yup.string().required('Please Enter email address'),
  ...
  onSubmit: async (values) => {
    ...
    try {
      ...
      setItem('name', response.data.userName);
       setItem('token',`Bearer ${response.data.token}`)
     
          Toast.show({
                type: 'success',
                text1: 'Success',
                text2: `${values.mobileOrEmail} OTP sent successfully`,
                position: 'top',
              });
      

        await Resendvalue(values.mobileOrEmail);

        // Navigate to OTP screen
        setItem('email',values.mobileOrEmail)
        setItem('otpverified','false')
        navigation.navigate('Loginotpverificationscreen', {
          mail: values.mobileOrEmail,
        });
    ...
    const Resendvalue = async (email) => {
          ...
          try {
            ...
            const payload = { Email: email, OtpType: 'SI' };
            const response = await ForgotMutation(payload); // <-- Replace this with your actual API function
